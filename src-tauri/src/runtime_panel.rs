...
pub(crate) fn run_calibrated_ocr_task(
    ...
    let screenshot_path = screenshot_path.map(Path::to_path_buf).unwrap_or_default();
    log_ocr_stage(
        paths,
        trace_id,
        "ocr-inference-start",
        format!(
            ...
            display_optional_path(path_if_present(&screenshot_path)),
    ...
    let (report, recognizer_report) =
        ocr::with_cached_ppocr_recognizer(&prepared.runtime_root, |recognizer| {
            ocr::recognize_calibrated_name_slots_from_dynamic_image(
                recognizer,
                &dictionary,
                &calibration,
                screenshot_image,
